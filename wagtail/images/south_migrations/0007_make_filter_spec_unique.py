...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        ...
        Filter = orm.Filter
        Rendition = orm.Rendition
        UserRendition = orm.UserRendition

        # find all the filters with duplicate specs
        #
        filters = Filter.objects.values('spec') \
                        .annotate(Count('pk')) \
                        .filter(pk__count__gt=1)
        for dup in filters:
            # get all the filters, select one to keep, delete the rest
            #
            spec = dup['spec']
