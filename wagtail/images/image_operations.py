...
class WidthHeightOperation(Operation):
    def construct(self, size, force=False):
        self.size = int(size)
        self.force = force
...
class ForceWidthHeightOperation(WidthHeightOperation):
    def construct(self, size, force=True):
        super().construct(size, force)


class CropRectangleOperation(Operation):
    def construct(self, rect):
...
class ForceFitOperation(Operation):
    """
    Resize down image to fill the given dimensions. Most suitable
