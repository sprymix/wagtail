...
class SaveAndLeaveMenuItem(ActionMenuItem):
    name = 'action-save-and-leave'
    label = _("Save and Leave")
...
class ArchiveMenuItem(ActionMenuItem):
    ...
    def is_shown(self, request, context):
        return (
            ...
            and context['user_page_permissions'].for_page(context['page']).can_archive()
            and not context['page'].archived
        )

    def get_url(self, request, context):
        return reverse('wagtailadmin_pages:archive', args=(context['page'].id,))


class UnarchiveMenuItem(ActionMenuItem):
    name = 'action-unarchive'
    label = _("Save and Unarchive")
